...
M.H.E. thanks the NSF, Division of Earth Sciences (grant EAR-8352055) and the following contributors to his Presidential Young Investigator Award for partial support of this research: Arco, Exxon, Phillips Petroleum, Texaco Inc., The Upjohn Co. We also acknowledge the donors of the Petroleum Research Fund, administered by the American Chemical Society (grant 16144-AC2 to M.H.E., grant 14805-AC2 to S.A.M.) for support. S.A.M. acknowledges NSERC (grant A2644) for partial support.
...
The total input, $x_{j}$ , to unit j is a linear function of the outputs, $y_{i}$ , of the units that are connected to j and of the weights, $w_{ji}$ , on these connections

$$
x _ {j} = \sum_ {i} y _ {i} w _ {j i} \tag {1}
$$
...
A unit has a real-valued output, $y_{j}$ , which is a non-linear function of its total input

$$
y _ {j} = \frac {1}{1 + e ^ {- x _ {j}}} \tag {2}
$$

![](images/8cda7eef5d88cd5ed1f26c025d9ac8b0f41180abaf820ed8753bea975beb0112.jpg)
...
```mermaid
graph TD
    A["Input units"] --> B["Hidden unit 1"]
    B --> C["Output unit 6.4"]
    C --> D["Input units 14.2"]
    D --> E["Output unit -8.8"]
    E --> F["Output unit -8.8"]
    F --> G["Output unit -1.1"]
    G --> H["Input units 14.2"]
    H --> I["Output unit -1.1"]
    I --> J["Output unit -1.1"]
    J --> K["Output unit -3.6"]
    K --> L["Output unit -7.2"]
    ...
    X --> Y["Output unit -1.1"]
    Y --> Z["Output unit -1.1"]
    Z --> AA["Output unit -3.6"]
    AA --> AB["Output unit -7.2"]
    AB --> AC["Output unit -1.1"]
    AC --> AD["Output unit -1.1"]
    AD --> AE["Output unit -3.6"]
    AE --> AF["Output unit -7.2"]
    AF --> AG["Output unit -1.1"]
    AG --> AH["Output unit -1.1"]
```
</details>
...
$$
E = \frac {1}{2} \sum_ {c} \sum_ {j} (y _ {j, c} - d _ {j, c}) ^ {2} \tag {3}
$$
...
![](images/6d295e9b79ba936abf007beb4005d97676f51657a806603ffbaa2970fa154783.jpg)
...
```mermaid
graph TD
    A["Christopher = Penelope"] --> B["Margaret = Arthur"]
    C["Andrew = Christine"] --> D["Victoria = James"]
    D --> E["Colin"]
    D --> F["Charlotte"]
    G["Roberto = Maria"] --> H["Gina = Emilio"]
    I["Pierro = Francesca"] --> J["Lucia = Marco"]
    J --> K["Alfonso"]
    J --> L["Sophia"]
    M["Jennifer = Charles"] --> N["Angela = Tomaso"]
```
</details>
...
![](images/ba717e33950de776e125ff15df5171a86ba6c459f13ffa5870648f709bf77d85.jpg)

<details>
<summary>natural_image</summary>

Abstract geometric pattern composed of black and white squares (no text or symbols)
</details>

Fig. 3 Activity levels in a five-layer network after it has learned. The bottom layer has 24 input units on the left for representing $\langle person 1\rangle$ and 12 input units on the right for representing the relationship. The white squares inside these two groups show the activity levels of the units. There is one active unit in the first group representing Colin and one in the second group representing the relationship 'has-aunt'. Each of the two input groups is totally connected to its own group of 6 units in the second layer. These groups learn to encode people and relationships as distributed patterns of activity. The second layer is totally connected to the central layer of 12 units, and these are connected to the penultimate layer of 6 units. The activity in the penultimate layer must activate the correct output units, each of which stands for a particular $\langle person 2\rangle$ . In this case, there are two correct answers (marked by black dots) because Colin has two aunts. Both the input units and the output units are laid out spatially with the English people in one row and the isomorphic Italians immediately below.
...
$$
\partial E / \partial y _ {j} = y _ {j} - d _ {j} \tag {4}
$$

We can then apply the chain rule to compute $\partial E / \partial x_{j}$

$$
\partial E / \partial x _ {j} = \partial E / \partial y _ {j} \cdot \mathrm{d} y _ {j} / \mathrm{d} x _ {j}
$$

Differentiating equation (2) to get the value of $dy_{j}/dx_{j}$ and substituting gives

$$
\partial E / \partial x _ {j} = \partial E / \partial y _ {j} \cdot y _ {j} (1 - y _ {j}) \tag {5}
$$
...
$$
\begin{array}{l} \partial E / \partial w _ {j i} = \partial E / \partial x _ {j} \cdot \partial x _ {j} / \partial w _ {j i} \\ = \partial E / \partial x _ {j} \cdot y _ {i} \tag {6} \\ \end{array}
$$

and for the output of the $i^{th}$ unit the contribution to $\partial E/\partial y_{i}$

![](images/7d49fea9bdef05827688365ed83e2660af6f40ea80996241cd82ed505f4a4653.jpg)
...
![](images/53f049b8fd325454739808cbe1e56b18556019359db50a7993def81f4a2c8a7d.jpg)
...
graph TD
    ...
    A["Set of corresponding weights"] --> B1 & B2 & B3 & D1 & D2 & D3
...
To break symmetry we start with small random weights. Variants on the learning procedure have been discovered independently by David Parker (personal communication) and by Yann Le Cun $^{4}$ .
...
![](images/4856b044c91d478b3fd9c5e5a1254fe442b714764ae692f2d201c42e2e1a7824.jpg)

<details>
<summary>scatter</summary>

| Cell Line | Days since termination of reverse occlusion | Spatial frequency (cycles per degree) |
| --------- | ------------------------------------------- | -------------------------------------- |
| C155      | 0                                           | 2.2                                    |
| C155      | 5                                           | 1.5                                    |
| C155      | 10                                          | 1.4                                    |
